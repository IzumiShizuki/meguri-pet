...
def main() -> int:
    ...
    custom_config = config_payload.setdefault("custom", {})
    if args.t2s_weights:
        custom_config["t2s_weights_path"] = args.t2s_weights
    if args.vits_weights:
        custom_config["vits_weights_path"] = args.vits_weights

    # GPT-SoVITS persists the last loaded weights. Use an isolated config so an
    # adapter preview cannot silently change the checked-in base configuration.
    with tempfile.TemporaryDirectory(prefix="meguri_tts_infer_") as temp_dir:
        runtime_config_path = Path(temp_dir) / source_config.name
        runtime_config_path.write_text(
            yaml.safe_dump(config_payload, allow_unicode=True, sort_keys=False),
            encoding="utf-8",
        )
        config = TTS_Config(str(runtime_config_path))
        pipeline = TTS(config)
        model_loaded_gpu_memory_mib = (
            round(torch.cuda.memory_allocated() / 1024**2, 3) if torch.cuda.is_available() else 0.0
        )
        results = []
            ...
            results.append(
                {
                    ...
                    "peak_gpu_allocated_mib": round(torch.cuda.max_memory_allocated() / 1024**2, 3)
                    if torch.cuda.is_available()
                    else 0.0,
                    "peak_gpu_reserved_mib": round(torch.cuda.max_memory_reserved() / 1024**2, 3)
                    if torch.cuda.is_available()
                    else 0.0,
    ...
    metrics = {
        ...
        "reference": reference,
        "seed": seed,
        "t2s_weights": str(config.t2s_weights_path),
        "vits_weights": str(config.vits_weights_path),
