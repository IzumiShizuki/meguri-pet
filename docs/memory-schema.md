...
Schema head: `20260728_0006`. PostgreSQL with pgvector is a hard prerequisite; migration `20260714_0001` fails explicitly if the extension cannot be created. No HNSW index exists in the baseline.
...
| `memory_candidates` | Pre-authority review queue | Typed status, memory type, sensitivity, source kind, risk level, merge policy and optional base version; provenance retained |
...
| `memory_embeddings` | Version/model/revision vectors | Unique version/model/revision; fixed dimension 2048; content SHA-256 |
...
5. `20260725_0005`: clear the rebuildable legacy BGE-M3 projection and switch pgvector to Model Studio's 2048-dimensional `text-embedding-v4` release.
6. `20260728_0006`: add candidate risk level, merge policy and optimistic base-version reference for reviewed merges.
