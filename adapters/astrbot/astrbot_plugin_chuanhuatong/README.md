...
This directory is an AGPL-3.0-compatible fork of bvzrays'
`astrbot_plugin_chuanhuatong` v2.3.0. The original layout editor, textbox,
components, fonts, attribution, repository link, and license are retained.

The Meguri adaptation adds these runtime-only integrations:

- consumes `meguri_render_payload` produced by `astrbot_plugin_meguri_gateway`;
- accepts the canonical five-field Meguri JSON as a compatibility fallback;
- treats Core's resolved outfit/expression as authoritative and only allows
  outfits `01` through `06`;
- selects versioned Meguri sprites and ordinary unpacked backgrounds from
  `plugin_data`, including day/evening/night and pool/shrine event variants;
- uses the unpacked original `MessageFrame` UI (`FRM_0101` plus its advance
  indicator) for Meguri structured replies, without stacking the generic
  glass frame or legacy name decorations;
- renders bilingual learning replies with the faithful Chinese translation
  above the natural Japanese original, one complete pair per image, without
  applying the legacy vertical image merge;
- falls back to the original renderer or plain text if assets are unavailable.

The canonical reply text inside the existing five-field JSON is:

```text
【<faithful Chinese translation>】
【<natural Japanese original>】
```

Repeat pairs with one blank line between them. Each pair is positional: the
first bracketed line is the Chinese translation and the second is the natural
Japanese original. The renderer rejects visible language labels or stray prose
so they are never silently dropped. The Chinese translation line uses
`ziti/Loli2.ttf` (萝莉体 第二版), while the Japanese original uses
`ziti/NotoSansJP-VF.ttf` so kana and Japanese kanji do not fall back to
missing-glyph boxes. Its variable-font weight is set by
`meguri_japanese_font_weight` (default `600`) to match the Chinese line. If
either file is absent, the renderer safely falls back to the existing body font.
Bilingual panels use their own
`meguri_bilingual_font_size` setting (default `42`, clamped to
`18`–`64`) without changing the legacy single-language body size.

Large character/background assets are intentionally not stored in the plugin
ZIP. Build them with `adapters/astrbot/build_meguri_gal_assets.ps1` and deploy
the generated release under:

```text
/AstrBot/data/plugin_data/astrbot_plugin_chuanhuatong/meguri_assets/releases/<release-id>
```

Set `meguri_asset_root` to that exact release directory. The LLM JSON does not
contain filenames, outfits, or background IDs; those remain runtime decisions.
