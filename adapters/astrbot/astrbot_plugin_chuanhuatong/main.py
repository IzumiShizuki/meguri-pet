...
@register(
    ...
    "bvzrays / Meguri adaptation",
    "传话筒：兼容 Meguri 结构化表情、服饰和时段场景的 Gal 风对话框",
    "2.4.0-meguri.5",
...
class ChuanHuaTongPlugin(Star):
    ...
    DEFAULT_EMOTIONS: list[dict[str, Any]] = [
        {"key": "neutral", "folder": "neutral", "label": "平静", "color": "#A9C5FF", "enabled": True},
        {"key": "happy", "folder": "happy", "label": "开心", "color": "#FFC857", "enabled": True},
        {"key": "affectionate", "folder": "affectionate", "label": "亲近", "color": "#F7B5D3", "enabled": True},
        {"key": "excited", "folder": "excited", "label": "兴奋", "color": "#FFD166", "enabled": True},
        {"key": "angry", "folder": "angry", "label": "生气", "color": "#FF8A8A", "enabled": True},
        {"key": "confused", "folder": "confused", "label": "困惑", "color": "#B8B8FF", "enabled": True},
        {"key": "embarrassed", "folder": "embarrassed", "label": "害羞", "color": "#F9C5D1", "enabled": True},
        {"key": "sad", "folder": "sad", "label": "低落", "color": "#7DA1FF", "enabled": True},
        {"key": "sleepy", "folder": "sleepy", "label": "困倦", "color": "#9EA9D4", "enabled": True},
        {"key": "surprised", "folder": "surprised", "label": "惊讶", "color": "#F5E960", "enabled": True},
        {"key": "teasing", "folder": "teasing", "label": "捉弄", "color": "#E4A7F2", "enabled": True},
        {"key": "worried", "folder": "worried", "label": "担心", "color": "#8EC5D6", "enabled": True},
    ]
    ...
    def __init__(self, context: Context, config: Optional[AstrBotConfig] = None):
        ...
        self._meguri_asset_root = self._resolve_meguri_asset_root()
        self._meguri_assets: Optional[MeguriAssetResolver] = None
        if self._cfg_bool("meguri_enabled", True) and self._meguri_asset_root.exists():
            manifest_value = str(self.cfg().get("meguri_manifest_path", "")).strip()
            manifest_path = (
                (
                    Path(manifest_value)
                    if Path(manifest_value).is_absolute()
                    else self._meguri_asset_root / manifest_value
                )
                if manifest_value
                else self._meguri_asset_root / "meguri_asset_manifest.json"
            )
            try:
                self._meguri_assets = MeguriAssetResolver(
                    self._meguri_asset_root,
                    manifest_path,
                )
            except Exception as exc:
                logger.warning("[传话筒] Meguri 素材解析器初始化失败: %s", exc)
        ...
        logger.info("[传话筒] Meguri 素材目录：%s", self._meguri_asset_root)
    ...
    def _resolve_meguri_asset_root(self) -> Path:
        raw = str(self.cfg().get("meguri_asset_root", "")).strip()
        if not raw:
            return self._data_dir / "meguri_assets" / "current"
        configured = Path(raw)
        if configured.is_absolute():
            return configured.resolve()
        return (self._data_dir / configured).resolve()

    def _resolve_meguri_game_ui_asset(self, config_key: str, default_name: str) -> Path | None:
        """Return a release-owned game UI asset without permitting path escape."""

        if not self._cfg_bool("meguri_game_ui_enabled", True):
            return None
        name = str(self.cfg().get(config_key, default_name) or default_name).strip()
        if not name:
            return None
        candidate = (self._meguri_asset_root / name).resolve()
        try:
            candidate.relative_to(self._meguri_asset_root.resolve())
        except ValueError:
            return None
    ...
    def _apply_meguri_name_layout(self, layout: Dict[str, Any]) -> Dict[str, Any]:
        """Keep the saved textbox geometry but replace its legacy character name."""

        adjusted = copy.deepcopy(layout)
        name = str(self.cfg().get("meguri_character_name", "爱莉")).strip() or "爱莉"
        name_chars = list(name[:2])
        legacy_name_ids = (
            "ov_1764055563203",
            "ov_1764055643818",
            "ov_1764055668857",
        ...
        overlays = adjusted.get("text_overlays", [])
        indexed = {
            str(overlay.get("id")): overlay
            for overlay in overlays
            if isinstance(overlay, dict)
        }
        for index, overlay_id in enumerate(legacy_name_ids):
            overlay = indexed.get(overlay_id)
            if overlay is None:
                continue
            overlay["text"] = name_chars[index] if index < len(name_chars) else ""
            overlay["visible"] = index < len(name_chars)
        adjusted["bot_name"] = name
        return adjusted
    ...
    async def _render_with_fallback(
        ...
        meguri_payload: Optional[dict[str, Any]] = None,
    ) -> Optional[str]:
        try:
            return await asyncio.to_thread(
                self._render_pillow_panel,
                text,
                emotion,
                session_id,
                meguri_payload,
    ...
    def _render_pillow_panel(
        ...
        normalized_payload = (
            normalize_gateway_payload(meguri_payload)
            if isinstance(meguri_payload, dict)
            else None
        )
        if normalized_payload is not None:
            layout = self._apply_meguri_name_layout(layout)
        ...
        bg_path = ""
        if normalized_payload is not None and self._meguri_assets is not None:
            resolved_background = self._meguri_assets.background(
                normalized_payload,
                session_id,
            )
            bg_path = str(resolved_background) if resolved_background else ""
        if not bg_path:
        ...
        game_ui_frame = (
            self._resolve_meguri_game_ui_asset("meguri_game_ui_frame", "ui/FRM_0101.png")
            if normalized_payload is not None
            else None
        )
        game_ui_indicator = (
            self._resolve_meguri_game_ui_asset("meguri_game_ui_indicator", "ui/FRM_0105.png")
            if game_ui_frame is not None
            else None
        )
        use_game_ui = game_ui_frame is not None

        layers: list[tuple[int, dict[str, Any]]] = []
        char_path = ""
        if normalized_payload is not None and self._meguri_assets is not None:
            resolved_character = self._meguri_assets.sprite(normalized_payload)
            char_path = str(resolved_character) if resolved_character else ""
        if not char_path:
        ...
        if char_path:
            character_z_index = max(
                int(layout.get("character_z_index", 150)),
                int(layout.get("meguri_game_ui_z_index", 130)) + 1,
            )
            layers.append((character_z_index, {"kind": "character", "path": char_path}))
        if use_game_ui:
            layers.append(
                (
                    int(layout.get("meguri_game_ui_z_index", 130)),
                    {
                        "kind": "meguri_game_ui",
                        "frame": game_ui_frame,
                        "indicator": game_ui_indicator,
                    },
                )
            )
        layers.append(
            (
                200 if use_game_ui else int(layout.get("textbox_z_index", 200)),
                {"kind": "textbox", "text": text, "meguri_game_ui": use_game_ui},
            )
        )
        if not use_game_ui:
        ...
        for _, layer in layers:
            ...
            elif kind == "meguri_game_ui":
                self._draw_meguri_game_ui_layer(
                    canvas,
                    layer.get("frame"),
                    layer.get("indicator"),
                )
            elif kind == "textbox":
                self._draw_textbox_layer(
                    canvas,
                    layout,
                    layer.get("text", ""),
                    use_meguri_game_ui=bool(layer.get("meguri_game_ui")),
                )
            elif kind == "text":
    ...
    def _meguri_game_ui_geometry(
        ...
        try:
            ...
            logger.debug("[传话筒] 游戏对话框尺寸读取失败", exc_info=True)
            return None
        bottom_margin = max(0, round(canvas.height * 0.02))
        top = max(0, canvas.height - target_height - bottom_margin)
        return 0, top, target_width, target_height

    def _draw_meguri_game_ui_layer(
        self,
        canvas: Image.Image,
        frame_path: Path | None,
        indicator_path: Path | None,
    ) -> None:
        if frame_path is None:
            return
        geometry = self._meguri_game_ui_geometry(canvas, frame_path)
        if geometry is None:
            return
        left, top, width, height = geometry
        try:
            with Image.open(frame_path) as frame:
                frame = frame.convert("RGBA").resize((width, height), Image.LANCZOS)
                canvas.alpha_composite(frame, (left, top))
            if indicator_path is None:
                return
            with Image.open(indicator_path) as indicator:
                scale = width / 1280
                indicator_size = max(12, round(32 * scale))
                indicator = indicator.convert("RGBA").resize(
                    (indicator_size, indicator_size), Image.LANCZOS
                )
                canvas.alpha_composite(
                    indicator,
                    (
                        width - round(indicator_size * 1.7),
                        top + height - round(indicator_size * 1.7),
                    ),
                )
        except Exception:
            logger.debug("[传话筒] 游戏对话框渲染失败", exc_info=True)
    ...
    def _draw_textbox_layer(
        self,
        canvas: Image.Image,
        layout: Dict[str, Any],
        text: str,
        *,
        use_meguri_game_ui: bool = False,
    ):
        ...
        if self._cfg_bool("meguri_bilingual_enabled", True):
            bilingual_pairs = parse_bilingual_pairs(text)
            if bilingual_pairs is not None:
                text = format_bilingual_pairs(bilingual_pairs)

        if bilingual_pairs is not None:
            bilingual_size = normalize_bilingual_font_size(
                self.cfg().get("meguri_bilingual_font_size", 42)
            )
            font = self._load_bilingual_font(bilingual_size, layout)
        else:
            font = self._load_font(layout.get("font_size", 30), preferred=layout.get("body_font"))
        if use_meguri_game_ui:
            frame_path = self._resolve_meguri_game_ui_asset(
                "meguri_game_ui_frame", "ui/FRM_0101.png"
            )
            geometry = (
                self._meguri_game_ui_geometry(canvas, frame_path)
                if frame_path is not None
                else None
            )
            if geometry is not None:
                _, frame_top, frame_width, frame_height = geometry
                box_left = round(frame_width * 0.325)
                box_top = frame_top + round(frame_height * 0.19)
                box_width = max(20, frame_width - box_left - round(frame_width * 0.045))
                box_height = max(20, frame_height - (box_top - frame_top) - round(frame_height * 0.08))
    ...
    def _draw_bilingual_textbox(
        ...
        *,
        ...
        """Draw translation/original lines with fonts suited to each language."""

        cursor_y = box_top + padding
        draw = ImageDraw.Draw(Image.new("RGBA", (max(10, text_area_width), 10)))
        ...
        for pair_index, (chinese, japanese) in enumerate(pairs):
            for content, font in (
                ...
                wrapped = self._wrap_text(content, font, max(10, text_area_width))
                line_spacing = max(0, int(getattr(font, "size", 24) * 0.12))
                self._draw_rich_text(
                    canvas,
                    (box_left + padding, cursor_y),
                    wrapped,
                    font,
                    fill,
                    stroke_width=stroke_width,
                ...
                cursor_y += max(getattr(font, "size", 24), bbox[3] - bbox[1]) + language_gap
    ...
    def _load_bilingual_font(
        self,
        size: int,
        layout: Dict[str, Any],
    ) -> ImageFont.FreeTypeFont | ImageFont.ImageFont:
        """Load the bundled Japanese-capable font, then safely use the old body font."""

        preferred = str(
            self.cfg().get("meguri_bilingual_font", "NotoSansJP-VF.ttf") or ""
        ).strip()
            ...
            try:
                ...
                try:
                    ...
                    for index, axis in enumerate(axes):
                        ...
                        if str(axis_name).casefold() == "weight":
                            values[index] = weight
                            font.set_variation_by_axes(values)
                            break
                except Exception:
                    logger.debug("[传话筒] 日文可变字重设置失败，沿用字体默认字重。", exc_info=True)
                return font
            except Exception:
                logger.debug("[传话筒] 双语字体加载失败，回退到原正文样式。", exc_info=True)
        return self._load_font(size, preferred=layout.get("body_font"))

    def _load_meguri_chinese_font(
        self,
        size: int,
        layout: Dict[str, Any],
    ) -> ImageFont.FreeTypeFont | ImageFont.ImageFont:
        """Load the requested Chinese display font while keeping a safe fallback."""

        preferred = str(self.cfg().get("meguri_chinese_font", "Loli2.ttf") or "").strip()
        resolved = self._resolve_font_path(preferred) if preferred else None
        if resolved:
            try:
    ...
    def _split_text_with_emotion(
        ...
        # A translation and its Japanese original are one semantic unit. Split
        # only between complete pairs, never between their two display lines.
        bilingual_chunks = (
            split_bilingual_text(clean_text, max_chars)
            if self._cfg_bool("meguri_bilingual_enabled", True)
            else None
        )
        chunks = bilingual_chunks or self._smart_split_text(clean_text, max_chars)
    ...
    async def _render_split_text(
        ...
        is_bilingual = (
            self._cfg_bool("meguri_bilingual_enabled", True)
            and parse_bilingual_pairs(text) is not None
        )
        ...
        if is_bilingual:
            ...
            for idx, (chunk_text, chunk_emotion) in enumerate(chunks_with_emotion, 1):
                ...
                try:
                    ...
                    if image_path:
                        self._schedule_cleanup(image_path, delay=5.0)
                    logger.debug("[传话筒] 发送第 %s 个双语面板失败", idx, exc_info=True)
            return delivered == len(chunks_with_emotion)
        ...
        # Every bilingual learning pair remains its own outbound image. The
        # legacy vertical merge would recreate an overflowing multi-pair panel.
        enable_merge = self._cfg_bool("merge_split_images", True) and not is_bilingual
    ...
    async def get_emotion_tag(self, event: AstrMessageEvent, resp: LLMResponse):
        ...
        if raw_text:
            parsed = parse_meguri_json(raw_text)
            if parsed is not None:
                cleaned_text, meguri_payload = parsed
                normalized_payload = normalize_gateway_payload(meguri_payload)
                if normalized_payload is not None:
                    event.set_extra("meguri_render_payload", normalized_payload)
                    emotion_tag = normalized_payload["expression"]["expression_tag"]
                else:
                    emotion_tag = "neutral"
                logger.debug("[传话筒] 已解析 Meguri 五字段 JSON")
            else:
                # 兼容原插件的 &emotion& 文本标签。
    ...
    async def on_decorating_result(self, event: AstrMessageEvent):
        ...
        meguri_payload_raw = event.get_extra("meguri_render_payload")
        meguri_payload = (
            normalize_gateway_payload(meguri_payload_raw)
            if isinstance(meguri_payload_raw, dict)
            else None
        )
        ...
        if resp_obj is None and meguri_payload is None and not allow_non_llm:
        ...
        if not has_non_text and raw_text_parts:
            ...
            emotion = (
                meguri_payload["expression"]["expression_tag"]
                if meguri_payload is not None
                else event.get_extra("extracted_emotion_tag")
            )
            emotion = emotion or "neutral"
            # 文本已经在handle_llm_response中清洗过，直接使用
            full_text = raw_full_text.strip()
            bilingual_pairs = None
            if meguri_payload is not None and self._cfg_bool("meguri_bilingual_enabled", True):
                bilingual_pairs = parse_bilingual_pairs(full_text)
                if bilingual_pairs is not None:
                    full_text = format_bilingual_pairs(bilingual_pairs)
            if full_text:
                ...
                # Multiple bilingual pairs always render as ordered, separate
                # panels, regardless of the legacy character threshold.
                if bilingual_pairs is not None and len(bilingual_pairs) > 1:
                    ...
                    if success:
                        result.chain = []
                        logger.info(
                            "[传话筒] 双语回复按 %s 个中日对完成分图渲染，会话: %s",
                            len(bilingual_pairs),
                            session_id,
                        )
                    else:
                        logger.warning("[传话筒] 双语分图渲染失败，返回规范化文本，会话: %s", session_id)
                    return
