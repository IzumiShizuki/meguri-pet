...
Final local results after the resumed memory audit:

- `python -m pytest -q`: **201 passed** against the live loopback pgvector dev
  database, with temporary read-only canonical data/asset junctions;
- direct Node test runner for the TypeScript protocol/renderer/AIRI/TTS/Website
  suite: **20 passed**;
- Python compile-all and Alembic offline upgrade/downgrade through
  `20260714_0004`: **passed**;
- deterministic exact-vector baseline: **p50 14.949 ms, p95 15.521 ms,
  p99 16.337 ms, 0% errors, recall@5 100%** on 500 synthetic vectors (not a
...
- release manifest and exposure ledger checks: **passed**;
- blocked staging acceptance checker: **returned 1 as expected**;
- blocked production approval checker: **returned 1 as expected**.
